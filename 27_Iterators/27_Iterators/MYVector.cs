...
public class MYVector<T> : MyList<T>
{
    ...
    public int Size { get { return elementCount; } set { elementCount = value; } } 
    ...
    public MYVector (MyCollection<T> a)
    {
        ...
        if (a.Size > 1000) throw new ArgumentException("Длина вектора a не должна превышать 1000 элементов");
        elementCount = a.Size; this.capacityIncrement = 0; 
        elementData = a.ToArray(); this.capacity = a.Size;
    }
    ...
    private void NewSize()
    {
        T[] newAr = capacityIncrement == 0 ?
            new T[Capacity * 2] : new T[Capacity + capacityIncrement];
        for (int i = 0; i < elementCount; i++) newAr[i] = elementData[i];
        elementData = newAr;
        Capacity = newAr.Length;
    ...
    public void AddAll(MyCollection<T> array) 
    {
        ...
        while (array.Size + elementCount > Capacity) NewSize();
        for (int i=0; i < array.Size; ++i) 
            elementData[elementCount++] = array.Get(i);
    ...
    public void Clear ()
    {
        this.elementData = new T[0]; this.elementCount = 0; this.Capacity = 0;
    }
    ...
    public bool ContainsAll (MyCollection<T> array)
    {
        ...
        if (array.Size == 0) throw new ArgumentException ("array");


        foreach (T item in array.ToArray())
    ...
    public bool Remove (object item)
    {
        ...
        if (found) { this.elementCount -=1; this.elementData = newV; return true; }
    ...
    public void RemoveAll (MyCollection<T> array)
    {
        ...
        {
            flag = false;
            for (int j = 0; j < array.Size || flag; j++)
                if (t.Equals(array.Get(j))) flag = true;
    ...
    public void RetainAll(MyCollection<T> array)
    {
        ...
        foreach (T t in this.elementData)
            foreach (T a in array.ToArray()) if (a != null && t.Equals(a)) 
                { newAr[index++] = t; break; }
    ...
    public void Add (int index,  T value) // вставка перед эл с заданным индексом
    {
    ...
    public class MYItr<E> : MYListIterator<E>
    {
        ...
        public MYItr(MYVector<E> mVector, int index = 0)
        {
            Cursor = index;
        ...
        public bool HasNext()
        {
            if (Cursor<mVector.elementCount) return true;
        ...
        public E Next()
        {
            if (HasNext()) return mVector.elementData[Cursor + 1];
        ...
        public bool Remove()
        {
            if (mVector.Remove(Cursor) is E el) return true;
        ...
        {
            if (Cursor + 1 < mVector.elementCount) return Cursor + 1;
        ...
        public E Previous()
        {
            if (HasPrevious()) return mVector.elementData[Cursor - 1];
