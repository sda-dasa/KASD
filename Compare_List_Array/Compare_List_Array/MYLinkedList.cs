...
namespace MYLinkedList
{
    ...
    public class MYLinkedList<T> : IEnumerable<T>
    {
        ...
        public void Add(int index, T item)
        {
            ...
            if (index == Size - 1) { AddLast(item); return; }
            Node<T> current = first;
            Node<T> node = new Node<T>(item); int i = 0;
            while (current != null)
            {
                if (index == ++i)
                {
                    node.Next = current.Next; node.Prev = current; current.Next.Prev = node;
        ...
        public void Clear()
        {
            first = default; last = default; size = 0;
        }
        ...
        public T PeekLast()
        {
            if (IsEmpty() || last is null) return default;
            return last.Info;
