...
namespace MYArray
{
    public class MyArrayList_<T>
    {
        ...
        public MyArrayList_() // создание пустого массива // можно без this
        {
            this.size = 0;
            this.Сapacity = 0;
        ...
        // можно так: public MyArrayList_(int capacity = 0) { this.size = 0; this.Capacity = capacity; this.elementData = new T [this.Capacity];} вместо двух конструкторов
        ...
        {
            if (a == null) throw new ArgumentNullException("array a"); //ArgumentNullException.ThrowIfNull(a);
        ...
        public void Add(int index, T el)
        {
            ...
            T[] newAr = new T[2*Size()];

            for (int i = 0; i < index; i++) newAr[i] = elementData[i];
            newAr[index] = el;
            for (int i = index + 1; i < Size(); i++) newAr[i] = elementData[i - 1];

            elementData = newAr; size++;
