...
namespace ClassMTM
{
    ...
    internal class MYTreeMap<K, V> 
    {
        ...
        private bool ContainsValue(object val,  TreeNode<K, V> root)
        {
            ...
            if (!(root is null))
            {
                if (!(root.Value == null) && root.Value.Equals(val)) return true;
        ...
        {
            if (keyend == null) throw new ArgumentNullException(nameof(keyend));
        ...
        */
        ...
        {
            if (keystart == null) throw new ArgumentNullException(nameof(keystart));
